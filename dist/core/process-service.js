...
export class ProcessService {
    ...
    startProcess(options) {
        ...
        if (useStdinPrompt && childProcess.stdin) {
            ...
            try {
                childProcess.stdin.write(cmd.stdinPrompt);
        ...
        childProcess.stdout?.on('data', (data) => {
            ...
            if (e)
                e.stdout += data.toString();
        });
        childProcess.stderr?.on('data', (data) => {
            const e = this.processManager.get(pid);
            if (e)
                e.stderr += data.toString();
        });
    ...
    async peekProcesses(pids, peekTimeSec = 10, includeToolCalls = false) {
        ...
        const terminalPromise = Promise.all(runningObservers.map((o) => this.waitForProcessTerminal(o.entry)));
        let timeoutHandle;
